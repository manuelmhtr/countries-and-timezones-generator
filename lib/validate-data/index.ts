...
const checkAllCountriesHaveTimezones = ({
	...
	if (diff.length > 0) {
		throw new Error(`There are ${diff.length} countries with no timezone: ${diff.join(', ')}`);
	}
};
...
const checkUtcOffsets = ({timezones}: CountriesAndTimezonesData) => {
	...
	if (errors.length > 0) {
		throw new Error(`There are ${errors.length} timezones without UTC offset: ${format(errors)}`);
	}
};
...
const checkAllAliasesExist = ({timezones}: CountriesAndTimezonesData) => {
	...
	if (errors.length > 0) {
		throw new Error(`There are ${errors.length} timezones with no alias: ${format(errors)}`);
	}
};
...
const checkAllCountriesAreValid = ({
	...
	if (errors.length > 0) {
		throw new Error(`There are ${errors.length} invalid countries: ${format(errors)}`);
	}
};
